...
struct pkcs7_message *pkcs7_parse_message(const void *data, size_t datalen)
{
	...
	ctx = calloc(1, sizeof(struct pkcs7_parse_context));
	if (!ctx)
		goto out_no_ctx;
	ctx->msg = calloc(1, sizeof(struct pkcs7_message));
	if (!ctx->msg)
		goto out_no_msg;
	ctx->sinfo = calloc(1, sizeof(struct pkcs7_signed_info));
	if (!ctx->sinfo)
		goto out_no_sinfo;
